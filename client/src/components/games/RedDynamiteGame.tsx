...
export const RedDynamiteGame: React.FC<RedDynamiteGameProps> = ({
  room,
  nightMode,
  onLeave,
  onNextGame,
  ...
  useEffect(() => {
      ...
      if (holder) {
        ...
        for (let i = 0; i < 150; i++) {
          particlesRef.current.push(
            new Particle(holder.x, holder.y, "#ff9800", "#f44336", 300),
          );
        }
      }
    });
    ...
    room.onMessage("dynamite_passed", (data) => {
      ...
      if (toPlayer) {
        for (let i = 0; i < 15; i++) {
          particlesRef.current.push(
            new Particle(toPlayer.x, toPlayer.y, "#f44336", "#ffeb3b", 100),
          );
        }
      }
    });
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      treesRef.current.forEach((tree) => {
        ...
        ctx.beginPath();
        ctx.arc(tree.x + 5, tree.y + 10, tree.radius, 0, Math.PI * 2);
        ctx.fill();
      ...
      if (gameState.players) {
        ...
        playersArray.forEach((player) => {
          ...
          if (player.hasDynamite) {
            const isFlashing =
              gameState.currentDynamiteTimer < 3 &&
          ...
          ctx.fillText(
            ...
            0,
            -s / 2 - 15 - (player.hasDynamite ? 25 : 0),
          );
      ...
      <div className="w-full flex justify-between items-center mb-4 px-4">
        ...
        >
          {gameState.roundState === "explosionDelay" ? (
            ...
            <span>
              TIMER:{" "}
              {Math.max(0, gameState.currentDynamiteTimer || 0).toFixed(1)}
            </span>
          )}
        </div>
      </div>
        ...
        >
          ...
          <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
            {getScoreboard().map((p, i) => (
              <div
                key={p.id}
                className={`flex items-center gap-2 p-2 border-2 ${!p.isAlive ? "opacity-50 grayscale" : ""} ${nightMode ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-slate-50"}`}
