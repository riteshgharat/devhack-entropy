...
async function runTest() {
  ...
  try {
    ...
    console.log(
      `✅ Joined successfully! Room ID: ${room.id} | Session ID: ${room.sessionId}`,
    );
    ...
    room.onStateChange((state) => {
      ...
      if (player) {
        console.log(
          `📍 Position: (${player.x.toFixed(2)}, ${player.y.toFixed(2)}) | Alive: ${player.isAlive} | Survivors: ${state.aliveCount}`,
        );
      }
    });
