...
import { Send, MessageCircle, Zap, Mic, MicOff, PhoneCall, PhoneOff, Users } from 'lucide-react';
...
const VoiceParticipantRow: React.FC<VoiceParticipantRowProps> = ({ name, speaking, muted, nightMode, isSelf }) => (
  <div className={`flex items-center gap-1.5 px-1 py-0.5 rounded text-[10px] transition-colors ${
    speaking
      ? (nightMode ? 'bg-emerald-900/50 text-emerald-300' : 'bg-emerald-50 text-emerald-700')
      : (nightMode ? 'text-slate-400' : 'text-slate-500')
  }`}>
    {/* Speaking ring */}
    <div className={`relative w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${
      speaking ? 'border-emerald-400' : (nightMode ? 'border-slate-600' : 'border-slate-300')
    }`}>
      {speaking && (
        <motion.div
      ...
      {muted
        ...
        : <Mic size={7} className={speaking ? 'text-emerald-400' : (nightMode ? 'text-slate-500' : 'text-slate-400')} />
      }
    </div>
...
export const CommunicationHub: React.FC<Props> = ({ room, nightMode, mySessionId, onOverlay, voiceSettings }) => {
  ...
  // ─── Voice Chat ───────────────────────────────────────────
  const { peers: voicePeers, isJoined: voiceJoined, isMuted, isSpeaking, speakingMap,
    join: joinVoice, leave: leaveVoice, toggleMute } = useVoiceChat(room, mySessionId);
  ...
  useEffect(() => {
    const handlers = [
      ...
      room.onMessage('ai_emoji_burst', (data: { emoji: string; target: string }) => {
  ...
  return (
    <div className={`relative flex flex-col gap-1 h-full rounded border-2 overflow-hidden ${bg}`} style={{ minWidth: 240, maxWidth: 260 }}>
      ...
      {/* ── Voice Channel ─────────────────────────────────── */}
      <div className={`shrink-0 px-2 pt-2 pb-1 border-b ${nightMode ? 'border-slate-700' : 'border-slate-200'}`}>
        {/* Header row */}
        <div className="flex items-center gap-1 mb-1.5">
          <Users size={10} className={nightMode ? 'text-emerald-400' : 'text-emerald-600'} />
          <span className={`font-display text-[9px] uppercase tracking-widest ${nightMode ? 'text-emerald-400' : 'text-emerald-600'}`}>
          ...
          <div className="ml-auto flex gap-1">
            ...
            {voiceJoined && (
              <motion.button
                ...
                className={`w-6 h-6 flex items-center justify-center rounded border transition-colors ${
                  isMuted
                    ? (nightMode ? 'border-red-600 bg-red-900/60 text-red-400' : 'border-red-400 bg-red-50 text-red-600')
                    : (nightMode ? 'border-emerald-600 bg-emerald-900/40 text-emerald-300' : 'border-emerald-500 bg-emerald-50 text-emerald-600')
                }`}
              >
            ...
            <motion.button
              ...
              className={`w-6 h-6 flex items-center justify-center rounded border transition-colors ${
                voiceJoined
                  ? (nightMode ? 'border-red-600 bg-red-900/60 text-red-400 hover:bg-red-900' : 'border-red-400 bg-red-50 text-red-500 hover:bg-red-100')
                  : (nightMode ? 'border-slate-600 bg-slate-700 text-slate-400 hover:border-emerald-500 hover:text-emerald-400' : 'border-slate-300 bg-white text-slate-500 hover:border-emerald-500 hover:text-emerald-600')
              }`}
            >
        ...
        {voiceJoined ? (
          <div className="flex flex-col gap-0.5">
            ...
            {voicePeers.length === 0 && (
              <p className="text-[9px] italic text-slate-500">Waiting for others to join...</p>
            )}
          </div>
        ) : (
          <p className={`text-[9px] ${nightMode ? 'text-slate-500' : 'text-slate-400'}`}>
      ...
      <div className="flex flex-col flex-1 overflow-hidden px-2 pt-2">
        <div className={`flex items-center gap-1 mb-1 font-display text-[9px] uppercase tracking-widest ${nightMode ? 'text-slate-400' : 'text-slate-500'}`}>
        ...
        <div className="flex-1 overflow-y-auto flex flex-col gap-0.5 pr-0.5" style={{ scrollbarWidth: 'thin' }}>
          {chat.map(msg => {
            ...
            return (
              <div 
                ...
                className={`text-[10px] leading-snug wrap-break-word ${
                  isAI 
                    ? (nightMode ? 'text-indigo-400 bg-indigo-950/40 px-2 py-1 rounded border-l-2 border-indigo-600' : 'text-indigo-700 bg-indigo-50 px-2 py-1 rounded border-l-2 border-indigo-400')
                    : isMine 
                      ? (nightMode ? 'text-emerald-300' : 'text-emerald-600')
          ...
          {chat.length === 0 && (
            <p className="text-[9px] text-slate-500 italic">No messages yet...</p>
          )}
          <div ref={chatEndRef} />
        </div>
      </div>
      ...
      {/* ── Divider ───────────────────────────────────────── */}
      <div className={`mx-2 border-t ${nightMode ? 'border-slate-700' : 'border-slate-200'}`} />
