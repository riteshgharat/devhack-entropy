...
import { Trophy, Timer, Users, Zap, AlertTriangle, Rocket, Bomb, Wind } from 'lucide-react';
...
export const GameArena: React.FC<GameArenaProps> = ({ room, nightMode, onLeave }) => {
    ...
    useEffect(() => {
        ...
        const render = (now: number) => {
            ...
            const grassBarGrad = ctx.createLinearGradient(barX, 0, barX + barWidth * fillRatio, 0);
    ...
    function drawPixelGrass(ctx: CanvasRenderingContext2D, x: number, y: number, night: boolean) {
        ctx.save();
    ...
    function drawPixelStickman(
        ...
        if (stunned) {
            ...
            ctx.fillText('💫', px + Math.cos(ang) * 16 - 6, py - 25 + Math.sin(ang) * 5);
            ctx.fillText('💫', px + Math.cos(ang + Math.PI) * 16 - 6, py - 25 + Math.sin(ang + Math.PI) * 5);
            ctx.fillText('💫', px + Math.cos(ang + Math.PI / 2) * 14 - 6, py - 28 + Math.sin(ang + Math.PI / 2) * 4);
            ctx.globalAlpha = 1.0;
        }
    ...
    function drawStickmanBody(ctx: CanvasRenderingContext2D, px: number, py: number, color: string, isLocal: boolean) {
    ...
    function drawBombEffect(ctx: CanvasRenderingContext2D, vfx: VFX) {
        ...
        if (progress < 0.3) {
            ...
            // Inner glow
            const innerGrad = ctx.createRadialGradient(vfx.x, vfx.y, 0, vfx.x, vfx.y, radius * 0.6);
            innerGrad.addColorStop(0, `rgba(255, 200, 50, ${0.8 * (1 - explosionProgress)})`);
            innerGrad.addColorStop(0.5, `rgba(239, 68, 68, ${0.5 * (1 - explosionProgress)})`);
            ...
            for (let i = 0; i < 12; i++) {
                ...
                ctx.fillStyle = i % 3 === 0 ? '#ef4444' : i % 3 === 1 ? '#fbbf24' : '#f97316';
    ...
    function drawRocketProjectile(ctx: CanvasRenderingContext2D, rocket: RocketProjectile) {
        ctx.save();
        ctx.imageSmoothingEnabled = false;
    ...
    const getScoreboard = () => {
        ...
        const entries: { name: string; score: number; id: string; color: string }[] = [];
        ...
        <div className="relative w-full max-w-5xl mx-auto mt-4 flex flex-col items-center">
            ...
            <div className="w-full flex justify-between items-start mb-3 px-2 gap-4">
                ...
                <div className="flex flex-col gap-2">
                    <div className={`px-4 py-2 border-4 flex items-center gap-3 font-display ${nightMode
                    ...
                    <div className={`px-4 py-2 border-4 flex items-center gap-3 font-display text-sm ${nightMode
                ...
                <AnimatePresence>
                    {lastEvent && (
                        <motion.div
                            ...
                            className={`px-5 py-3 border-4 font-display text-xs uppercase tracking-wider text-center max-w-xs ${nightMode
                                ? 'bg-slate-900/95 border-yellow-500/80 text-yellow-300'
                                : 'bg-yellow-50/95 border-yellow-500 text-yellow-800'
                                }`}
                        >
                            <div className="flex items-center gap-2 justify-center">
                                {eventIcon === 'bomb' && <Bomb size={16} className="text-red-500" />}
                                {eventIcon === 'rocket' && <Rocket size={16} className="text-orange-500" />}
                                {eventIcon === 'speed' && <Zap size={16} className="text-yellow-400" />}
                                <span>{lastEvent}</span>
                            </div>
                        </motion.div>
                    )}
                ...
                {/* Right: Scoreboard */}
                <div className={`border-4 font-display text-xs ${nightMode
                    ? 'bg-slate-900/90 border-indigo-500/80 text-slate-200'
                    : 'bg-white/90 border-slate-700 text-slate-800'
                    }`}
                    style={{ minWidth: '180px' }}
                >
                    <div className={`px-3 py-1 text-center uppercase tracking-widest border-b-2 ${nightMode ? 'border-indigo-700 bg-indigo-900/50' : 'border-slate-300 bg-slate-100'
                        }`}>
                    ...
                    <div className="px-3 py-1">
                        {getScoreboard().map((entry, i) => (
                            <div key={entry.id}
                                className={`flex items-center gap-2 py-1 ${entry.id === room.sessionId ? 'font-bold' : ''
                                    }`}
                            >
                                ...
                                <span className="text-green-400 text-[10px]">{entry.score}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {/* Canvas Container */}
            <div className={`relative border-8 ${nightMode
