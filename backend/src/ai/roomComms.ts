...
export class RoomComms {
  ...
  private _registerHandlers() {
    this.room.onMessage('chat_message', (client: any, payload: any) => {
      ...
      const displayName = player?.displayName || `Player_${client.sessionId.slice(0, 4)}`;
    ...
    this.room.onMessage('voice_offer', (client: any, payload: any) => {
      ...
      if (target) {
        target.send('voice_offer', { from: client.sessionId, sdp: payload.sdp });
      }
    });
    ...
    this.room.onMessage('voice_answer', (client: any, payload: any) => {
      const target = this._findClient(payload?.to);
      if (target) {
        target.send('voice_answer', { from: client.sessionId, sdp: payload.sdp });
      }
    });
    ...
    this.room.onMessage('voice_ice', (client: any, payload: any) => {
      const target = this._findClient(payload?.to);
      if (target) {
        target.send('voice_ice', { from: client.sessionId, candidate: payload.candidate });
      }
    });

    /** Broadcast speaking status (throttled by client) */
    this.room.onMessage('voice_speaking', (client: any, payload: any) => {
      this.room.broadcast('voice_speaking', {
        sessionId: client.sessionId,
  ...
  private _findClient(sessionId: string): any | null {
    return this.room.clients.find((c: any) => c.sessionId === sessionId) ?? null;
  }

  private _sendToOthers(excludeSessionId: string, type: string, data: any): void {
  ...
  private _buildAIInput(): AIInput {
    ...
    const leader = sorted[0] ? { name: sorted[0].displayName || 'Unknown', score: sorted[0].score ?? 0 } : null;
    const weakest = sorted[sorted.length - 1] && sorted.length > 1
      ? { name: sorted[sorted.length - 1].displayName || 'Unknown', score: sorted[sorted.length - 1].score ?? 0 }
      : null;
    ...
    return {
      ...
      chatHighlights: this.chat.slice(-5).map(c => `${c.displayName}: ${c.text}`),
    };
  }
  ...
  private _broadcastOutput(output: AIOutput) {
    ...
    if (output.arenaEvent && output.arenaEvent.type !== 'none') {
      ...
      console.log(`[RoomComms] Arena event broadcasted: ${output.arenaEvent.type}`);
    }
  }
}
