        ...
        <div className="fixed inset-0 pointer-events-none z-[5]">
            {particles.map((p) => (
                <motion.div key={p.id} className="absolute"
                    style={{ left: `${p.x}%`, bottom: '-10px', width: `${p.size}px`, height: `${p.size}px`, backgroundColor: p.color, imageRendering: 'pixelated' }}
                    animate={{ y: [0, -window.innerHeight - 50], opacity: [0, 1, 1, 0], rotate: [0, Math.random() > 0.5 ? 90 : -90] }}
...
const LightningBolt: React.FC<{ x: number; delay: number }> = ({ x, delay }) => (
    <motion.div className="absolute pointer-events-none z-[8]"
        style={{ left: `${x}%`, top: 0, width: '3px', height: '100vh', background: 'linear-gradient(180deg, transparent, #fbbf24, transparent)', filter: 'blur(2px)' }}
        initial={{ opacity: 0, scaleY: 0 }}
        animate={{ opacity: [0, 1, 0], scaleY: [0, 1, 1] }}
        transition={{ delay, duration: 0.3, repeat: Infinity, repeatDelay: 4 + Math.random() * 3 }}
    />
);
        ...
        <div className="fixed inset-0 pointer-events-none z-[1]">
            {stars.map((star) => (
                <motion.div key={star.id}
                    style={{ position: 'absolute', left: `${star.x}%`, top: `${star.y}%`, width: star.size, height: star.size, backgroundColor: '#fff', imageRendering: 'pixelated' }}
                    animate={{ opacity: [0.3, 1, 0.3] }}
                    transition={{ duration: 2 + star.blink, repeat: Infinity, ease: 'easeInOut' }}
                />
            ))}
        </div>
    );
...
const PixelFire: React.FC<{ color: string; x: number }> = ({ color, x }) => {
    ...
    return (
        <div className="absolute pointer-events-none" style={{ left: `${x}%`, bottom: 0, zIndex: 15 }}>
            {flames.map((i) => (
                <motion.div key={i}
                    style={{ position: 'absolute', left: i * 5 - 15, bottom: 0, width: 4, height: 12, backgroundColor: i % 2 === 0 ? color : '#fbbf24', imageRendering: 'pixelated' }}
                    animate={{ scaleY: [1, 1.5, 0.8, 1.3, 1], opacity: [1, 0.8, 1, 0.6, 1] }}
...
/* ── Combo counter popup ── */
const ComboText: React.FC<{ text: string; show: boolean }> = ({ text, show }) => (
    <AnimatePresence>
        {show && (
            <motion.div
                className="absolute z-[50] left-1/2 -translate-x-1/2"
                style={{ top: '30%', fontFamily: '"Press Start 2P", system-ui', fontSize: '14px', color: '#fbbf24', textShadow: '3px 3px 0 #000, -1px -1px 0 #ef4444', whiteSpace: 'nowrap' }}
...
export const SplashScreen: React.FC<SplashScreenProps> = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        timers.push(setTimeout(() => { setPhase(1); setShakeKeyChars(k => k + 1); }, 800));
        timers.push(setTimeout(() => { setPhase(2); triggerCombo('ALL FIGHTERS READY!'); }, 2600));
        timers.push(setTimeout(() => { setPhase(3); setShakeKeyTitle(k => k + 1); }, 5000));
        ...
        <AnimatePresence>
            {phase < 5 && (
                <motion.div
                    ...
                    animate={phase >= 4 ? { scale: 2.5, opacity: 0, filter: 'blur(12px)', backgroundColor: '#050508' } : { scale: 1, opacity: 1, filter: 'blur(0px)' }}
                    transition={phase >= 4 ? { duration: 1.3, ease: [0.76, 0, 0.24, 1] } : { duration: 0.3 }}
                >
                    <ScanLines />
                    <StarField />
                    ...
                    <motion.div
                        ...
                        animate={{ x: [0, -14, 12, -8, 6, -3, 0], y: [0, 8, -12, 6, -4, 1, 0] }}
                    ...
                    >
                        ...
                        {/* ═══ CHARACTERS ═══ */}
                        <div className="absolute z-[10] w-full flex items-end justify-center" style={{ bottom: '12%' }}>

                            {/* Ground platform */}
                            <motion.div className="absolute bottom-0 left-[5%] right-[5%] z-[1]"
                                style={{ height: '6px', backgroundColor: '#1e293b', boxShadow: '0 4px 0 #0f172a, 0 8px 0 #0a0f1a' }}
                                initial={{ scaleX: 0 }} animate={phase >= 1 ? { scaleX: 1 } : {}}
                                transition={{ duration: 0.6, ease: 'easeOut' }}
                            />
                            {/* Ground tiles */}
                            {phase >= 1 && (
                                <div className="absolute bottom-0 left-[5%] right-[5%] z-[1]" style={{ height: '6px', display: 'flex', overflow: 'hidden' }}>
                                    {Array.from({ length: 30 }, (_, i) => (
                                        <div key={i} style={{ flex: 1, borderRight: '1px solid rgba(255,255,255,0.05)', height: '100%' }} />
                                    ))}
                                </div>
                            )}
                            ...
                            <AnimatePresence>
                                {phase >= 1 && (
                                    <motion.div style={{ zIndex: 10, marginBottom: '6px' }}
                                    ...
                                    >
                                        ...
                                        <motion.div className="absolute -bottom-3 left-1/2 -translate-x-1/2"
                                            style={{ width: '100px', height: '12px', borderRadius: '50%', border: '3px solid #ef4444', backgroundColor: 'transparent' }}
                                        ...
                                        <motion.div className="absolute -bottom-4 left-1/2 -translate-x-1/2"
                                            style={{ width: '80px', height: '16px', borderRadius: '50%', backgroundColor: '#ef4444', filter: 'blur(8px)', opacity: 0.5 }}
                                        ...
                                        >
                                            <PixelStickman color="#ef4444" scale={2} crown={true} weapon="sword" />
                                        </motion.div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            {phase >= 2 && (
                                <motion.div className="absolute" style={{ zIndex: 5, marginBottom: '6px' }}
                                    initial={{ x: -500, y: 0, opacity: 0 }}
                                    animate={{ x: -150, y: 0, opacity: 1 }}
                                    transition={{ duration: 0.65, ease: [0.34, 1.56, 0.64, 1] }}
                                >
                                    <motion.div style={{ width: '60px', height: '8px', backgroundColor: '#3b82f6', opacity: 0.3, filter: 'blur(4px)', position: 'absolute', left: -60, top: '60%' }}
                                        initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.3 }}
                                    />
                                    <motion.div animate={{ y: [0, -6, 0] }}
                                        transition={{ delay: 0.8, duration: 1.4, repeat: Infinity, ease: 'easeInOut' }}>
                                        <PixelStickman color="#3b82f6" scale={1.5} weapon="shield" />
                                    </motion.div>
                                </motion.div>
                            )}
                            ...
                            {phase >= 2 && (
                                ...
                                >
                                    <motion.div animate={{ y: [0, -6, 0] }}
                                        transition={{ delay: 1.0, duration: 1.2, repeat: Infinity, ease: 'easeInOut' }}>
                            ...
                            {phase >= 2 && (
                                <motion.div className="absolute" style={{ zIndex: 4, marginBottom: '6px' }}
                            ...
                            {phase >= 2 && (
                                ...
                                >
                                    <motion.div animate={{ y: [0, -5, 0] }}
                                        transition={{ delay: 1.3, duration: 1.3, repeat: Infinity, ease: 'easeInOut' }}>
                        ...
                        {/* Red bottom glow */}
                        {phase >= 1 && (
                            <motion.div className="fixed bottom-0 left-0 right-0 pointer-events-none z-[3]"
                                style={{ height: '40%', background: 'linear-gradient(to top, rgba(239,68,68,0.12) 0%, transparent 100%)' }}
                                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}
                            />
                        )}
                    </motion.div>{/* ← end character shake wrapper */}
                    ...
                    >
                            ...
                            {phase >= 3 && (
                                ...
                                >
                                    ...
                                    <motion.div className="flex items-center gap-3 mb-3"
                                        ...
                                        <div style={{ width: '60px', height: '4px', backgroundColor: '#ef4444' }} />
                                        <div style={{ width: '8px', height: '8px', backgroundColor: '#fbbf24', transform: 'rotate(45deg)' }} />
                                        <div style={{ width: '120px', height: '4px', backgroundColor: '#ef4444' }} />
                                        <div style={{ width: '8px', height: '8px', backgroundColor: '#fbbf24', transform: 'rotate(45deg)' }} />
                                        <div style={{ width: '60px', height: '4px', backgroundColor: '#ef4444' }} />
                                    </motion.div>
                                    ...
                                    <h1 style={{
                                        ...
                                        color: '#facc15',
                                        textShadow: '6px 6px 0 #000, -4px -4px 0 #ef4444, 10px 10px 20px rgba(0,0,0,0.5)',
                                        ...
                                        <br />
                                        <motion.span
                                            style={{ color: '#ef4444', textShadow: '6px 6px 0 #000, -4px -4px 0 #facc15' }}
                                            animate={{ textShadow: ['6px 6px 0 #000, -4px -4px 0 #facc15', '6px 6px 0 #000, -4px -4px 0 #a855f7', '6px 6px 0 #000, -4px -4px 0 #facc15'] }}
                                    ...
                                    {/* Subtitle */}
                                    <motion.p
                                        style={{ fontFamily: '"Press Start 2P"', fontSize: 'clamp(0.5rem, 1.5vw, 0.9rem)', color: 'rgba(255,255,255,0.6)', letterSpacing: '0.35em', marginTop: '12px' }}
                                    ...
                                    >
                                        <span style={{ fontFamily: '"Press Start 2P"', fontSize: 'clamp(0.55rem, 1.4vw, 0.85rem)', color: 'rgba(220,220,230,0.85)', letterSpacing: '0.12em' }}>
                                            Entering in arena..
                                        </span>
                                        <motion.span
                                            style={{ display: 'inline-block', width: '10px', height: '10px', backgroundColor: 'rgba(220,220,230,0.85)' }}
                        ...
                        {phase === 3 && (
                            <motion.div className="fixed inset-0 z-[30] pointer-events-none"
                                ...
                                initial={{ opacity: 0.9 }} animate={{ opacity: 0 }} transition={{ duration: 0.5 }}
                            />
                        )}
                    </motion.div>{/* ← end title shake wrapper */}
                </motion.div>
            )}
        </AnimatePresence>
    );
