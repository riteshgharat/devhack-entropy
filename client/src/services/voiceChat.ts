...
export class VoiceChat {
  ...
  private listeners: {
    [K in keyof VoiceEventMap]?: Listener<VoiceEventMap[K]>[];
  } = {};
  ...
  private _registerSignalingHandlers() {
    ...
    reg(
      "voice_peers",
      ({ peers }: { peers: { sessionId: string; displayName: string }[] }) => {
        console.log(
          "[VoiceChat] voice_peers — existing peers:",
          peers.map((p) => p.displayName),
        );
        peers.forEach((p) => {
    ...
    reg(
      "voice_speaking",
      ({ sessionId, speaking }: { sessionId: string; speaking: boolean }) => {
  ...
  private _getOrCreatePC(peerId: string): RTCPeerConnection {
    ...
    pc.ontrack = (evt) => {
      ...
      audio
        .play()
        .catch((e) => console.warn("[VoiceChat] Audio play blocked:", e));
    };
    ...
    pc.onicegatheringstatechange = () => {
      console.log(
        "[VoiceChat] ICE gathering state for",
        peerId,
        ":",
    ...
    pc.oniceconnectionstatechange = () => {
      console.log(
        "[VoiceChat] ICE connection state for",
        peerId,
        ":",
    ...
    pc.onconnectionstatechange = () => {
      console.log(
        "[VoiceChat] Connection state for",
        peerId,
        ":",
        pc.connectionState,
      );
  ...
  private _emit<K extends keyof VoiceEventMap>(
    event: K,
    payload: VoiceEventMap[K],
  ): void {
    (
      this.listeners[event] as Listener<VoiceEventMap[K]>[] | undefined
