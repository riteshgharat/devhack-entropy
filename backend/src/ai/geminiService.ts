import { AIInput, AIOutput, AICommentary, AIOverlay, ArenaEvent, AIEmojiBurst } from './types';
...
const FALLBACKS: AIOutput[] = [
  {
    commentary: [{ id: 'f1', text: "The arena pulses with raw energy — anything can happen!", tone: 'hype', createdAt: 0 }],
    overlay: { id: 'f1', title: 'CHAOS INCOMING!', severity: 'high', durationMs: 3500 },
    emojiBurst: { emoji: '🔥', target: 'all' },
    arenaEvent: { type: 'spawn_random_hazard' },
  },
  {
    commentary: [{ id: 'f2', text: "The leader is getting cocky. Time for a reality check.", tone: 'dramatic', createdAt: 0 }],
    overlay: { id: 'f2', title: 'TARGET: THE LEADER', subtitle: 'Everyone, rally together!', severity: 'medium', durationMs: 4000 },
    emojiBurst: { emoji: '🎯', target: 'leader' },
    arenaEvent: { type: 'spotlight_player' },
  },
  {
    commentary: [{ id: 'f3', text: "Tick tock tick tock... survival is getting spicy!", tone: 'tense', createdAt: 0 }],
    overlay: { id: 'f3', title: 'SUDDEN PRESSURE', subtitle: 'The arena shrinks...', severity: 'critical', durationMs: 4500 },
    emojiBurst: { emoji: '😱', target: 'all' },
    arenaEvent: { type: 'shrink_boundary' },
  },
  {
    commentary: [{ id: 'f4', text: "LOL look at them go! Pure madness.", tone: 'funny', createdAt: 0 }],
    overlay: { id: 'f4', title: 'SPEED RUSH!', severity: 'medium', durationMs: 3000 },
    emojiBurst: { emoji: '⚡', target: 'all' },
    arenaEvent: { type: 'speed_up' },
  },
  {
    commentary: [{ id: 'f5', text: "Everything... s l o w s... d o w n.", tone: 'dramatic', createdAt: 0 }],
    overlay: { id: 'f5', title: 'SLOW MOTION!', severity: 'medium', durationMs: 3000 },
    emojiBurst: { emoji: '🐢', target: 'all' },
...
export function getFallback(): AIOutput {
  ...
  return {
    ...fb,
    commentary: fb.commentary.map(c => ({ ...c, id: `fb_${now}`, createdAt: now })),
...
function buildPrompt(input: AIInput): string {
  const leaderStr = input.leader ? `${input.leader.name} (${input.leader.score} pts)` : 'none yet';
  const weakestStr = input.weakest ? `${input.weakest.name} (${input.weakest.score} pts)` : 'none';
  const eventStr = input.recentEvents.length > 0 ? input.recentEvents.slice(-5).join('; ') : 'nothing notable';
  const chatStr = input.chatHighlights.length > 0 ? input.chatHighlights.slice(-3).join(' | ') : '(quiet)';
  const gameContext = GAME_CONTEXT[input.roomType] ?? 'A chaotic multiplayer arena battle.';
...
export async function callGemini(input: AIInput): Promise<AIOutput> {
  ...
  try {
    ...
    const commentary: AICommentary[] = (parsed.commentary ?? []).map((c: any, i: number) => ({
      id: `ai_${now}_${i}`,
      text: String(c.text ?? '').slice(0, 140),
      tone: ['hype', 'tense', 'funny', 'dramatic'].includes(c.tone) ? c.tone : 'hype',
      createdAt: now,
    }));
    ...
    const overlay: AIOverlay | null = rawOverlay
      ? {
          id: `aio_${now}`,
          title: String(rawOverlay.title ?? 'CHAOS!').slice(0, 30).toUpperCase(),
          subtitle: rawOverlay.subtitle ? String(rawOverlay.subtitle).slice(0, 60) : undefined,
          severity: ['low', 'medium', 'high', 'critical'].includes(rawOverlay.severity) ? rawOverlay.severity : 'medium',
          durationMs: Math.min(8000, Math.max(1500, Number(rawOverlay.durationMs ?? 3000))),
        }
      : null;

    const rawBurst = parsed.emojiBurst;
    const emojiBurst: AIEmojiBurst | null = rawBurst
      ? { emoji: String(rawBurst.emoji ?? '🔥'), target: ['all', 'leader', 'weakest'].includes(rawBurst.target) ? rawBurst.target : 'all' }
      : null;

    const ALLOWED_EVENTS = new Set(['none', 'shrink_boundary', 'spawn_random_hazard', 'speed_up', 'slow_mo', 'spotlight_player']);
    ...
    console.log(`[AI] Gemini response received → ${commentary[0]?.text ?? '(empty)'}`);
    return { commentary, overlay, emojiBurst, arenaEvent };
  } catch (err: any) {
    clearTimeout(timeoutId);
    console.warn(`[AI] Gemini call failed (${err?.message ?? err}), using fallback`);
    return getFallback();
  }
}
