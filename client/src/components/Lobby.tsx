...
export const Lobby = () => {
  return (
    <PixelCard title="Lobby" className="w-full max-w-sm">
      <div className="space-y-3 mt-2">
        {players.map((player) => (
          <div 
            key={player.id} 
            className="flex items-center gap-3 bg-slate-100 p-2 border-2 border-slate-300 hover:bg-white transition-colors cursor-pointer group"
          >
            {/* Avatar */}
            ...
            <div className="flex-1">
              <div className="font-display text-xs text-slate-800">{player.name}</div>
              <div className="font-body text-slate-500 text-sm leading-none">Lvl. {Math.floor(Math.random() * 50) + 1}</div>
            </div>
        ...
        {/* Empty Slot */}
        <div className="flex items-center justify-center gap-2 p-3 border-2 border-dashed border-slate-300 text-slate-400 font-body text-lg hover:bg-slate-50 hover:border-slate-400 hover:text-slate-500 transition-colors cursor-pointer">
      ...
      <div className="mt-4 pt-4 border-t-2 border-slate-200 flex justify-between items-center">
        <div className="flex gap-2">
           ...
           <span className="font-body text-slate-600">4/8 Players</span>
        </div>
        <div className="font-display text-[10px] text-slate-400">Region: US-East</div>
      </div>
    </PixelCard>
  );
};
