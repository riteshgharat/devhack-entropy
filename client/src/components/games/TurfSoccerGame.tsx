...
export const TurfSoccerGame: React.FC<TurfSoccerGameProps> = ({
  room,
  nightMode,
  onLeave,
  onNextGame,
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      area.removeEventListener("touchmove", moveEvt);
  ...
  useEffect(() => {
      ...
      if (ball) {
        for (let i = 0; i < 80; i++) {
          particlesRef.current.push(
            new Particle(ball.x, ball.y, "#ffeb3b", "#fff", 200),
          );
        }
      }
    });
    ...
    room.onMessage("ball_reset", () => {
      ...
      for (let i = 0; i < 30; i++) {
        particlesRef.current.push(
          new Particle(FIELD_CX, FIELD_CY, "#90caf9", "#e3f2fd", 60),
        );
      }
    });
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      ctx.beginPath();
      ctx.moveTo(FIELD_CX, 0);
      ctx.lineTo(FIELD_CX, FH);
      ctx.stroke(); // Center line
      ctx.beginPath();
      ctx.arc(FIELD_CX, FIELD_CY, 80, 0, Math.PI * 2);
      ctx.stroke(); // Center circle
      ctx.fillStyle = FIELD_LINE;
      ctx.beginPath();
      ctx.arc(FIELD_CX, FIELD_CY, 5, 0, Math.PI * 2);
      ctx.fill(); // Center dot
      ...
      for (let yy = FIELD_CY - gHalf; yy < FIELD_CY + gHalf; yy += 12) {
        ctx.beginPath();
        ctx.moveTo(-GOAL_WIDTH, yy);
        ctx.lineTo(0, yy);
        ctx.stroke();
      }
      for (let xx = -GOAL_WIDTH; xx <= 0; xx += 12) {
        ctx.beginPath();
        ctx.moveTo(xx, FIELD_CY - gHalf);
        ctx.lineTo(xx, FIELD_CY + gHalf);
        ctx.stroke();
      }
      // Posts
      ...
      if (state?.players) {
        ...
        all.forEach((player: any) => {
          ...
          ctx.fillStyle =
            player.team === 1 ? "rgba(244,67,54,0.7)" : "rgba(33,150,243,0.7)";
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-7xl mx-auto">
      ...
      <div className="w-full flex justify-center items-center mb-3 px-4 gap-6">
        {/* Team 1 Score */}
        <div
          className={`flex items-center gap-3 px-6 py-2 border-4 font-display text-2xl ${nightMode ? "bg-red-900/40 border-red-600 text-red-300" : "bg-red-100 border-red-500 text-red-700"}`}
        >
          <span className="text-sm uppercase tracking-wider opacity-70">
            Red
          </span>
          <span className="text-3xl font-bold">
            {gameState?.scoreTeam1 ?? 0}
          </span>
        </div>
        {/* Timer */}
        <div
          className={`px-8 py-2 border-4 font-display text-3xl ${nightMode ? "bg-slate-800 border-slate-600 text-white" : "bg-white border-slate-400 text-slate-800"}`}
        >
          {Math.max(0, Math.ceil(gameState?.matchTimer ?? 90))}
        </div>
        {/* Team 2 Score */}
        <div
          className={`flex items-center gap-3 px-6 py-2 border-4 font-display text-2xl ${nightMode ? "bg-blue-900/40 border-blue-600 text-blue-300" : "bg-blue-100 border-blue-500 text-blue-700"}`}
        >
          <span className="text-sm uppercase tracking-wider opacity-70">
            Blue
          </span>
          <span className="text-3xl font-bold">
            {gameState?.scoreTeam2 ?? 0}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 w-full">
        ...
        >
              ...
              >
                <div className="text-center">
                  <motion.div
                    ...
                    style={{
                      textShadow:
                        "6px 6px 0 #000, -3px -3px 0 #4caf50, 0 0 30px rgba(255,255,255,0.5)",
                    }}
                  >
                    {gameState.countdown}
                  </motion.div>
          ...
          <AnimatePresence>
            {gameState?.roundState === "goal" && (
              ...
              >
                <div
                  className="font-display text-8xl text-yellow-400"
                  style={{
                    textShadow: "6px 6px 0 #000, 0 0 40px rgba(255,235,59,0.8)",
                  }}
                >
                  GOAL!
                </div>
        ...
        >
          ...
          <div className="mb-3">
            <div
              className={`font-display text-xs uppercase mb-1 ${nightMode ? "text-red-400" : "text-red-600"}`}
            >
              Team Red
            </div>
            {getScoreboard()
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className={`flex items-center gap-2 p-1.5 mb-1 border-2 ${nightMode ? "border-red-800 bg-red-900/30" : "border-red-200 bg-red-50"}`}
                >
                  <div
                    className="w-3 h-3"
                  ...
                  />
                  <span
                    className={`flex-1 truncate font-display text-xs ${nightMode ? "text-slate-200" : "text-slate-700"}`}
                  >
                    {p.name} {p.isBot ? "🤖" : ""}
                  </span>
                  <span className="font-bold font-display text-xs text-green-500">
                    {p.score}
                  </span>
                </div>
              ))}
          ...
          <div>
            <div
              className={`font-display text-xs uppercase mb-1 ${nightMode ? "text-blue-400" : "text-blue-600"}`}
            >
              Team Blue
            </div>
            {getScoreboard()
              ...
              .map((p) => (
                <div
                  key={p.id}
                  className={`flex items-center gap-2 p-1.5 mb-1 border-2 ${nightMode ? "border-blue-800 bg-blue-900/30" : "border-blue-200 bg-blue-50"}`}
                >
                  <div
                    className="w-3 h-3"
                    style={{ backgroundColor: p.color }}
          ...
          {myTeam > 0 && (
            <div
              className={`mt-auto pt-3 text-center font-display text-xs uppercase tracking-wider ${
                myTeam === 1
                  ? nightMode
                    ? "text-red-400"
                    : "text-red-600"
