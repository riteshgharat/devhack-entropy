...
export const GrassGame: React.FC<GrassGameProps> = ({ room, nightMode, onLeave, onNextGame }) => {
            ...
            if (prevGridRef.current.length > 0 && state.grid) {
                for (let i = 0; i < state.grid.length; i++) {
                    if (state.grid[i] < prevGridRef.current[i]) {
                        ...
                        const color1 = state.grid[i] === 1 ? PALETTE.grassLight : PALETTE.dirt;
                        for (let p = 0; p < 8; p++) {
                            particlesRef.current.push(new Particle(x, y, PALETTE.grassBase, color1));
                        }
                    }
                }
            }
            ...
            if (state.items) {
                state.items.forEach((item: any) => {
                    ...
                    if (wasActive && !item.active && item.type === 'mine') {
                        // Mine exploded
                        explosionsRef.current.push({ x: item.x, y: item.y, radius: 0, life: 1 });
                        for (let i = 0; i < 15; i++) {
                            particlesRef.current.push(new Particle(item.x, item.y, '#ff9800', '#f44336', 150));
                        }
                    } else if (wasActive && !item.active && item.type === 'booster') {
                        // Booster collected
                        for (let i = 0; i < 10; i++) {
                            particlesRef.current.push(new Particle(item.x, item.y, '#00bcd4', '#e0f7fa', 100));
                        }
                    }
        ...
        const render = (time: number) => {
            ...
            if (gameState.players) {
                ...
                playersArray.forEach((player) => {
                    ...
                    let wiggleY = 0;
                    if (!isStunned && (player.velocityX !== 0 || player.velocityY !== 0)) {
        ...
        <div className="flex flex-col items-center w-full max-w-6xl mx-auto">
            ...
            <div className="w-full flex justify-between items-center mb-4 px-4">
                <div className={`font-display text-2xl ${nightMode ? 'text-white' : 'text-slate-800'}`}>
                    TIME: {Math.ceil(gameState.countdown > 0 ? gameState.countdown : (gameState.matchEnded ? 0 : (gameState.matchTimer ?? 60)))}
                </div>
            </div>
            ...
            <div className={`w-full md:w-64 border-4 p-4 flex flex-col ${nightMode ? 'border-slate-700 bg-slate-800/80' : 'border-slate-300 bg-white/80'}`}>
                ...
                <div className="flex flex-col gap-2 flex-1 overflow-y-auto">
                    {getScoreboard().map((p, i) => (
                        <div key={p.id} className={`flex items-center gap-2 p-2 border-2 ${nightMode ? 'border-slate-600 bg-slate-900' : 'border-slate-200 bg-slate-50'}`} style={{ borderColor: p.color }}>
